...
from carpy.gaskinetics import GasModels, FlowState
    ...
    def airspeeds(self, altitude: Hint.nums = None, geometric=False, *,
        ...
        while True:
            ...
            # CAS can be computed from TAS
                ...
                Mach = TAS / a
                pt_p = 1 / FlowState.p_pt(M=Mach, gamma=gamma)
                qc = p * (pt_p - 1)
            ...
            # TAS can be computed from CAS
                ...
                # Use isentropic flow relations to find the Mach number
                M_sl = CAS / a_sl
                pt_psl = 1 / FlowState.p_pt(M=M_sl, gamma=gamma)
                qc = p_sl * (pt_psl - 1)
                Mach = FlowState.M(p_p0=(p / (p + qc)), gamma=gamma)
                TAS = Mach * a
